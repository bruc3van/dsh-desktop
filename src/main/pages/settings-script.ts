...
export function renderSettingsPageScript(chinese: boolean): string {
  ...
  return 'const $ = id => document.getElementById(id);'
    + 'let shownNotes="";'
    + 'let marketSaving=false;let marketReading=false;let marketEpoch=0;'
    ...
    + 'if(s.mode!=="local")line+=' + t(' 当前连接已有服务，此开关不会修改该服务；下次由客户端启动本地服务时生效。', ' Connected to an existing server; this switch takes effect on the next client-managed local launch.') + ';'
    ...
    + t('已保存。启动设置在下次由客户端启动本地服务时生效。', 'Saved. This setting takes effect on the next client-managed local launch.') + ';'
    + '}catch(e){$("market-note").textContent=' + t('保存失败：', 'Save failed: ') + '+e.message}'
    + 'finally{marketSaving=false;button.disabled=false;void refreshMarket()}};'
