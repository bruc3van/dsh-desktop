...
function writePnpmShim(binDir: string): void {
  const entry = join(process.resourcesPath, 'dsh-runtime', 'node_modules', 'pnpm', 'bin', 'pnpm.mjs')
  if (!existsSync(entry)) return
  const launcher = runtimeLauncher()
  if (process.platform === 'win32') {
    writeFileSync(join(binDir, 'pnpm.cmd'), [
      '@echo off',
      'rem Generated by DeepSeek Harness Desktop. The bundled pnpm CLI, named `pnpm`.',
      'setlocal',
      'set "ELECTRON_RUN_AS_NODE=1"',
      'set "DSH_DESKTOP_RUNTIME_ENTRY=' + entry + '"',
  ...
  writeFileSync(shim, [
    '#!/bin/sh',
    '# Generated by DeepSeek Harness Desktop. The bundled pnpm CLI, named `pnpm`.',
    'DSH_DESKTOP_RUNTIME_ENTRY="' + entry + '" ELECTRON_RUN_AS_NODE=1 \\',
