...
- 共享环境因插件解析、加载或激活失败而无法启动时，客户端会列出诊断中确认存在于 profile 的全部问题插件，让用户选择一次性卸载并用共享环境重试，或保留插件并使用桌面端独立环境；取消不会修改插件或数据。只有 profile 中真实存在的直接依赖才会进入卸载操作，且卸载后会核对依赖和 bundle 条目都已移除。独立环境使用自己的运行时锁和随机端口，不会把共享环境中运行在 3080 的实例误判成同一份数据的写入者。
