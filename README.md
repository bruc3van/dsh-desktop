...
- **Agent 执行环境经过工程治理。** Agent 本质是在你电脑上执行命令的进程，它的环境值得被认真对待：优先复用你正在跑的实例与 PATH 上的 `dsh`，Agent 跑在你自己的完整 shell 环境里；内置运行时下没装 Node.js 的用户也能执行 `node`；`ELECTRON_RUN_AS_NODE` 不会泄漏给 Agent（否则 `code` 等 Electron 工具会启动失败）；从 Finder/Dock 启动也能找到 Homebrew、`~/.local/bin` 和写在 `~/.zshrc` 里的工具。详见[内置运行时下的 Agent 执行环境](#内置运行时下的-agent-执行环境)。
...
- **插件先审查、再安装**：内置安全市场默认关闭、开启后才联网；「安全安装」不执行任何安装命令，而是把审查提示词交给 Agent，确认干净后再由你决定是否用官方命令安装；客户端只在内置运行时下接入市场、其他连接方式自动撤回，绝不把闭包插件交给另一份运行时。详见[内置安全市场](#内置安全市场)。
...
使用内置运行时时，客户端把随安装包发布的[安全市场](https://github.com/bruc3van/dsh-desktop-safe-market)（`dsh-desktop-safe-market`）按官方 in-box bundle 的方式接入当前 profile：只往 profile 的 `dsh.profile.bundles` 里加一项、并建一条指向运行时闭包的软链——不写依赖、不改锁文件、不跑 pnpm、不联网；升级插件即升级客户端。设置里随之出现「插件市场」导航项，分两页：「插件」页上半是已安装面板（查看、停用/启用、卸载当前 profile 通过包安装的插件），下半是社区市场；「技能」页列出当前会话实际能解析到的技能。这种接入只在内置运行时下存在：复用已有实例、用户自装的 dsh、或固定地址连接时会自动撤回，以免把闭包里的插件交给另一份运行时。
...
桌面端对这次接入与其他工程投入同样谨慎：接入失败可自愈——插件在加载期抛错会让**整棵插件树**启动失败、共用同一 profile 的 CLI 也会跟着起不来，因此失败即撤回并在本会话不再重试；插件目录缺失、或 `profiles/node_modules` 上已有非软链目录时不写 `bundles`，绝不写坏你的 profile；你自己装过该插件（作为 profile 依赖）且版本不低于内置时，客户端完全不插手；更旧的覆盖会改用闭包那份。市场的完整设计（配置项、目录协议、安全边界与已知限制）见其[仓库](https://github.com/bruc3van/dsh-desktop-safe-market)。
...
内置了：随安装包发布的[安全市场](https://github.com/bruc3van/dsh-desktop-safe-market)，设置里叫「插件市场」。它只在使用内置运行时时按官方 in-box bundle 的方式接入当前 profile（写入 `dsh.profile.bundles` 并建一条指向闭包的软链），市场默认关闭、开启后才联网；复用已有实例、用户自装的 dsh、或固定地址连接时会自动撤回，以免把闭包里的插件交给另一份运行时。目录的每日自动采集 + 人工精选、以及「先审查、再安装」的完整流程见[内置安全市场](#内置安全市场)。官方 Web UI 自身的能力（技能、插件、交互等）仍随官方发版直接出现在窗口里。桌面外壳自身的后续工作见[开发指南](docs/development.zh.md#当前状态)与 [TODO](TODO.md)：macOS/Windows 签名与公证、系统通知、OS Keychain、语音输入，以及内置运行时的独立更新通道、运行中实例的周期探测提示等。
