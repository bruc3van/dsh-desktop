...
[安全市场](https://github.com/bruc3van/dsh-desktop-safe-market)随安装包离线发布，在客户端启动且依赖兼容的运行时中接入；首次启动即可加载。复用已经运行的本机实例或连接自定义地址时，不改动其插件配置。用户通过官方 CLI 安装的市场由用户管理，客户端不会因版本较旧而替换它。
