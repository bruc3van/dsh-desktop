...
- **智能模式：复用你正在跑的实例。** 依次探测：`127.0.0.1:3080` 上的官方实例（含 `~/.dsh/profiles/web/cordis.patch.yml` 里配置的其他端口）→ PATH 上的 `dsh` → npx 缓存 → 内置运行时。浏览器、CLI 与桌面端共享同一个 Harness 进程，会话状态实时同步。DSH 0.1.2 起 Web API 要求浏览器会话认证；桌面端会使用同一 `DSH_HOME` 中官方持久化的会话签名记录，为本机回环地址生成一份短时 HttpOnly 会话，不保存终端打印的进程启动 token。
