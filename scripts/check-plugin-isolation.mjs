...
const pluginDiagnostic = "Error: dsh: plugin tree failed to load: failed to apply loader entry include (cordis:include): failed to import loader entry better-sidebar (dsh-better-sidebar): The requested module '@deepseek-ai/dsh-settings' does not provide an export named 'settingsNamespace'"
...
try {
  ...
  if (settings.dshDataFallbackPlugin !== 'dsh-better-sidebar') throw new Error('the failing plugin package was not persisted')
  ...
  if (!compatibilityNote?.includes('dsh-better-sidebar')) {
    throw new Error('recovery settings did not name the failing plugin: ' + compatibilityNote)
  }
  ...
  if ('dshDataFallbackReason' in restored || 'dshDataFallbackPlugin' in restored || 'dshDataFallbackNoticeShown' in restored) {
  ...
  console.log('✓ the recovery settings name the failing plugin package')
  ...
  if (installedSettings.dshDataFallbackPlugin !== 'dsh-better-sidebar') {
    throw new Error('installed-only fallback lost the failing plugin name\n' + installedLog)
  }
  ...
  if (npxSettings.dshDataFallbackPlugin !== 'dsh-better-sidebar') {
    throw new Error('npx-only fallback lost the failing plugin name\n' + npxLog)
  }
