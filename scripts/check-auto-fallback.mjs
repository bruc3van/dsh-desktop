/**
 * Smart-mode recovery integration check. A probed Web UI disappears, so the
 * desktop must start its managed runtime; that child is then killed once and
 * must be relaunched within the bounded recovery budget.
 */
...
const waitForStatus = async (app, predicate, timeoutMs) => {
  ...
  throw new Error('status condition timed out: ' + JSON.stringify(lastStatus))
}
