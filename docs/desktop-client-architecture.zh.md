...
- **内置插件接入**：市场发布包独立放在 `resources/bundled-plugins`，不出现在 DSH 安装目录的模块查找路径中。受客户端管理的 CLI 子进程在加载 profile 前检查所选 DSH 的版本和市场 peer 范围，调用该版本的官方 `initProfile()`，再把市场复制到 `<DSH_HOME>/profiles/web/node_modules/dsh-desktop-safe-market` 并登记 bundle。这样 bundle patch 和插件模块都从同一个 profile 包加载，DSH Service 则使用实际 runtime 的依赖。只更新带 `.dsh-desktop-seat.json` 标记的副本；用户依赖、锁文件和其他本地安装无论新旧都保留。旧共享副本会迁移到 web，其他 profile 仍引用的旧副本保留。复用或连接已有服务不改写 profile；无法验证的原始 CLI 不自动接入市场。禁用内置市场后，下次受管理的启动移除客户端拥有的登记与副本；用户安装不受影响。见 `src/main/bundled-market-boot.ts`、`src/main/bundled-plugin.ts`。
- **运行时锁定**：两个 harness 同时写同一份会话日志会造成永久损坏——各自的内存序号让日志出现重复 seq 与孤儿 inbox splice，受影响的会话再也无法恢复；而客户端不可能发现机器上的每一个 harness（官方 web 的端口既可来自 `--port`，也可写在 profile 补丁层，后者在命令行上留不下任何线索），所以排他不能依赖探测。客户端能可靠做到的是绝不同时跑两个**自己**的运行时：把自己 spawn 的子进程 pid、客户端 pid、启动时间与就绪后的源站记录在 `DSH_HOME` 下的 `.dsh-desktop-runtime.json`（经临时文件改名落盘——撕裂读会解析成「没有运行时」，那是唯一会以数据为代价的错误答案）。下次启动先读记录：仍在服务的遗留进程被优先接管（复用同一个 harness，会话照常共享），而不是在旁边再起一个——幸存者不是假设场景：Windows 安装器按进程名杀掉应用时会留下运行时子进程，新版再起一个就会成为同一 `DSH_HOME` 的第二个写入者。记录里还留着它出自哪一档来源：用户若已在连接设置里关掉那一档，客户端先停掉它再按新偏好解析，无法核实归属而仍可连接时继续接管；已经尝试终止却无法确认结果时保留阻塞记录——偏好不值得换来第二个写入者。完整记录可自愈：既不应答也不存在的子进程被清除；未完成的启动／停止占位记录则会阻止后继，直到使用者确认没有遗留进程；接不上的遗留进程先清理再重启，连不上也结束不掉时客户端拒绝启动并说明原因。更新路径遵循同一条约束：安装包下载并通过 SHA-256 校验之后、拉起安装器之前才停止运行时——下载或校验失败、安装器最终没能启动时运行时都会保住或自动恢复（此前 Windows 上安装器按进程名杀应用时匹配不到 `node.exe` 子进程，旧运行时成为孤儿继续写 `~/.dsh`）。见 `src/main/runtime-lock.ts`。
- **设置接缝**：带随机私密路径的最小回环页面承载原生连接窗口，并把 `settings.json` 写入 `~/.bruc3van-dsh-desktop`。它不是 API carrier，不代理 `/api`、WebSocket 或 renderer 资源。

客户端自己的设置放在 `~/.bruc3van-dsh-desktop`（可用 `DSH_DESKTOP_HOME` 覆盖）；本地子进程默认使用共享的官方 `~/.dsh`，也可选择独立的 `~/.bruc3van-dsh-desktop/dsh`。数据环境在进程生命周期内冻结，切换通过重启生效；共享环境若因插件启动失败，客户端从诊断中收集全部问题包名，并只把 profile 中确认为直接依赖且已启用的插件列入一次性卸载。用户也可保留插件并改用独立环境，或取消且不修改数据。窗口使用官方 logo（macOS 模板 Dock 图标、Windows/Linux 窗口图标）与标准标题栏（官方 Web UI 自带 header）。凭据接缝按原样使用：`DEEPSEEK_API_KEY` 经官方界面的设置写入。
...
完整本地桥及媒体、剪贴板读取、文件选择权限只授予客户端当前自有子进程的 Web UI。Connect 地址和仅经描述符探测发现的页面可以加载，但敏感变更须经原生确认，状态推送也不包含本地诊断。API 描述符只用于兼容性识别，不证明服务身份。

运行时启动前先写入 `launchPending` 占位记录，随后写入子进程 PID、来源和系统创建标识。写入失败会显示错误并尝试停止子进程，不自动启动后继。停止前同样持久化未完成标志；Windows 只执行并核对树终止，不退回到直接杀包装进程。成功确认退出才清除记录。若应用在占位期间崩溃，下次启动会拒绝猜测，需确认使用该数据目录的进程及其子进程全部退出后再删除提示中的记录。

Windows 进程身份使用完整 FILETIME；Linux 使用 boot ID 与启动 tick；macOS 使用 `ps` 的创建时间和程序名，其时间精度为秒。旧记录的近似年龄只能排除 PID 复用，不能授权终止；仍可连接的旧实例继续接管。重复退出等待同一个收尾 Promise，最多 15 秒，超时后仍执行现有同步退出兜底。

慢速 CLI 版本检测结束后以及实际请求启动前重新检查占用，探测默认包括 3080、13080 和配置端口。这缩小发现与启动之间的窗口，但没有与外部官方 CLI 共享的原子互斥锁，不能宣称消除了所有并发启动竞态。
