...
这四种来源都可以在连接设置里单独关闭（多选按钮），方便测试时固定到某一种；缺省全部开启，至少保留一种。未勾选的来源会被跳过，顺序不变。关掉「本机已运行」只表示客户端不再连接探测到的实例：客户端不会结束用户自己启动的进程。若该实例仍在应答，也不会在同一份会话数据上再启动本地运行时（以免双写损坏），而是提示先退出该实例再重试。`DSH_DESKTOP_SKIP_PROBE=1` 会同时跳过这道占用闸和复用连接。`DSH_DESKTOP_SKIP_INSTALLED_DSH=1` 仍只在开发环境生效，会跳过 PATH 与 npx 探测。
