...
- **Smart mode reuses the instance you are already running.** It probes in order: the official instance on `127.0.0.1:3080` (including other ports configured in `~/.dsh/profiles/web/cordis.patch.yml`) → a `dsh` on PATH → an npx-cached package → the bundled runtime. Browser, CLI, and desktop then share one live Harness process, with sessions synced in real time. DSH 0.1.2-alpha.1 and later authenticate every Web API request; the desktop uses the official persistent session-signing record in the same `DSH_HOME` to create a short-lived HttpOnly session for the loopback origin, without storing the process launch token printed in the terminal. If a running dsh still holds the secret from before its credential file changed, the desktop asks you to restart that instance instead of risking another writer on the same session data.
...
|---|---|---|
| **Smart** (default) | Most local users | Tries in order: the official instance on `127.0.0.1:3080` → a `dsh` on PATH → an npx-cached official package → the bundled runtime. Any of those four can be turned off in Connection settings. |
| **Custom** | A Web UI instance you maintain (official dsh currently targets local use only) | Connects to the Web UI address you provide and starts no runtime of its own. |
...
Smart mode uses only what **already exists** on your machine — a `dsh` on PATH or the official package npx has cached — nothing is downloaded, and Node.js is never installed for you. The official `dsh web` port can also be written in the web profile's patch layer (`~/.dsh/profiles/web/cordis.patch.yml`) besides the `--port` flag, and the client reads and probes those ports too — it will not start a second harness beside an instance you simply moved. A **client-started** dsh automatically tries port 3080 first, then 13080, then an OS-assigned port. Connection settings can instead pin its bind port (the change applies immediately; a taken pinned port is not replaced). When an npx-cached package is older than the bundled runtime, the connection settings say so (the cache still wins — it is what your last `npx @deepseek-ai/dsh` left behind; running it again refreshes it). Whatever the client starts is a plain background service; rc.8 and newer get `--no-open`, and the child is shut down when you quit; if the chosen runtime fails to start, the client walks the remaining enabled sources (the bundled runtime last, when it is still enabled). Each of the four sources can be turned off independently with toggle buttons in Connection settings, which is how a test run pins itself to one of them; all four are on by default, and at least one must stay on. While an official instance is still running and Smart mode will not reuse it, the client will not start an installed, npx, or bundled runtime beside it, and it will not terminate that process — quit it in the terminal first. If the bundled runtime cannot start, or you want another instance, open **Settings → Desktop settings**. The full runtime-resolution order is documented in the [development guide](docs/development.md#run-from-source).
...
| Status | Meaning |
|---|---|
| Already running | An instance you ran yourself (`127.0.0.1:3080` by default) |
| Client-started · bundled | A background child using the runtime shipped in the installer |
| Client-started · npx cache | A background child using the official package npx already cached |
| Client-started · installed | A background child using the `dsh` you installed on PATH |
| Custom address | A direct connection; the client starts no runtime |
...
Choose **Smart** to return to Smart mode. Connection settings live under **Desktop settings** in the official settings dialog.
...
- **Origin-tiered permissions.** The client-managed loopback Web UI may use notifications, clipboard reads/writes, fullscreen, camera/microphone, and user-initiated file pickers so the desktop shell does not break official UI or plugin capabilities. A custom remote target gets only clipboard writes and notifications. Cross-origin subframes, USB/HID/serial, geolocation, display capture, and storage escalation remain denied; unauthorized in-page navigation and new-window redirects are still blocked.
- **Notification platform boundary.** Installed Windows builds use the AppUserModelID registered by the installer; source runs use the current `electron.exe` identity as Electron recommends, though Windows may still require pinning that executable to Start to register its toast activator. Linux keeps Chromium's native desktop notifications. Because macOS Notification Center requires valid code signing, current ad-hoc-signed builds automatically preserve the same Web Notification contract through a Dock count badge, Dock bounce, and clickable in-app reminder. Clicking still returns to the source conversation; focusing the window or closing the reminder clears Dock attention.
...
- **Restart** in the tray menu restarts the local runtime along with the client, which is what a plugin that only takes effect on a fresh runtime needs. A runtime the client started is stopped and started again; an adopted leftover runtime is stopped too when its ownership can be verified. A `dsh web` you started yourself in a terminal is left running — there, only the client shell restarts. Pinning that same origin (Connect mode) also leaves it running: it is then a saved address, not an adopted Smart-mode instance.
...
- If plugins prevent the shared environment from starting, the client lists every diagnosed direct profile plugin and lets the user remove them together and retry Shared, keep them and use the isolated environment, or cancel without changing data. Removal is offered only for direct dependencies present in the profile and is verified against both dependencies and bundle entries afterward.
...
![Safe Market in settings: the curated catalog dealt evenly across categories, with the market's own version beside its title](docs/images/marketplace.png)
...
![The installed panel: version, enabled state, and the disable/enable and uninstall verbs](docs/images/marketplace-installed.png)

Uninstalling is not a strike-through in a list: the plugin is disabled first, then removed from this profile with the same primitive `dsh plugin remove` uses, so the lockfile and `node_modules` go with it. A plugin installed as a dependency but missing from `bundles` — installed, yet not loaded — is listed on its own and labelled as such: whatever is on your machine is visible here, and can be uninstalled right here.

The **Skills** page lists the skills the current session can actually resolve, each with its provider and invocation policy. It is addressed by session because skill discovery is layered by the agent preset a session runs — read from the plugin's root context it would see only the global layer and report "no skills" while a dozen are in reach.
...
Yes: the [safe marketplace](https://github.com/bruc3van/dsh-desktop-safe-market) shipped in the installer (shown in settings as **Safe Market**). It is installed the way official in-box bundles are (copied into `profiles/node_modules`, with its name in `dsh.profile.bundles`) for whichever runtime the client starts, and is off until enabled — only then does it talk to the network. A pinned address withdraws that entry, because the client does not start that runtime; reusing an already-running local instance re-seats it conservatively (name only, never a tree swap). The daily-collected, hand-curated catalog and the review-before-install flow are described in [The bundled safe marketplace](#the-bundled-safe-marketplace). Capabilities of the official Web UI itself (skills, plugins, interactions) still appear in the window as the official project ships. Work on the desktop shell itself is listed in the [development guide](docs/development.md#project-status) and [TODO](TODO.md): proper macOS/Windows signing and notarization, OS keychain integration, voice input, an independent update channel for the bundled runtime, and periodic probing of newly appeared instances.
...
- **[dsh-desktop-safe-market](https://github.com/bruc3van/dsh-desktop-safe-market)** — the review-before-install DSH marketplace. It powers the **Safe Market** bundled with this client: the catalog is collected daily and curated by hand, and Safe install hands a security-review prompt to the Agent, installing with the official command only after a clean reading. Configuration, the catalog protocol, and known limitations live in its repository.
