...
- **Restart** in the tray menu restarts the local runtime along with the client, which is what a plugin that only takes effect on a fresh runtime needs. A runtime the client started is stopped and started again; an adopted leftover runtime is stopped too when its ownership can be verified. A `dsh web` you started yourself in a terminal is left running — there, only the client shell restarts.
