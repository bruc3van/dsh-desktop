...
The client treats this seat with the same care as everything else it does: failure self-heals — a plugin that throws while loading fails the whole plugin tree (and the CLI sharing the profile), so a failed seat is withdrawn and not retried this session; the client never writes `bundles` when the plugin directory is missing or a non-symlink directory occupies `profiles/node_modules`, so your profile can never be corrupted by the seat; and if you installed the plugin yourself as a profile dependency, the client stays out of the way entirely. The market's full design (configuration, catalog protocol, security boundaries, and known limitations) lives in its [repository](https://github.com/bruc3van/dsh-desktop-safe-market).
